...
def target_pair_queries() -> list[QueryDefinition]:
    return [
        QueryDefinition(
            name="summary",
            description="Core patent and family counts for a target pair.",
            default_limit=1,
            cypher="""
MATCH (tp:TargetPair {name: $value})-[:HAS_TARGET_PAIR]-(p:Patent)-[:PUBLISHED_IN]-(y:Year)
...
""",
        ...
        QueryDefinition(
            name="yearly_trend",
            description="Yearly publication counts for a target pair.",
            default_limit=100,
            cypher="""
MATCH (tp:TargetPair {name: $value})-[:HAS_TARGET_PAIR]-(p:Patent)-[:PUBLISHED_IN]-(y:Year)
...
""",
        ...
        QueryDefinition(
            name="top_assignees",
            description="Top assignees for a target pair.",
            default_limit=20,
            cypher="""
MATCH (tp:TargetPair {name: $value})-[:HAS_TARGET_PAIR]-(p:Patent)-[:PUBLISHED_IN]-(y:Year)
...
""",
        ...
        QueryDefinition(
            name="origins",
            description="Origin distribution for assignees connected to a target pair.",
            default_limit=20,
            cypher="""
MATCH (tp:TargetPair {name: $value})-[:HAS_TARGET_PAIR]-(p:Patent)-[:PUBLISHED_IN]-(y:Year)
MATCH (p)-[:HAS_ASSIGNEE]-(a:Assignee)-[:ORIGIN_FROM]-(o:Origin)
WITH o, p, toInteger(y.year) AS year
WHERE year >= $year_min AND year <= $year_max
RETURN o.name AS origin,
       count(DISTINCT p) AS patent_count
ORDER BY patent_count DESC, origin
LIMIT $limit
""",
        ),
        QueryDefinition(
            name="annotations",
            description="Target, pathway, function, cancer, and technology annotations for a target pair.",
            default_limit=100,
            cypher="""
MATCH (tp:TargetPair {name: $value})-[:HAS_TARGET_PAIR]-(p:Patent)-[:PUBLISHED_IN]-(y:Year)
...
WITH t, pathway, cancer, f1, f2, f3, tc, toInteger(y.year) AS year
WHERE year >= $year_min AND year <= $year_max
RETURN collect(DISTINCT t.symbol) AS targets,
...
""",
        ...
        QueryDefinition(
            ...
            description="Representative patent publications for a target pair.",
            default_limit=50,
            cypher="""
MATCH (tp:TargetPair {name: $value})-[:HAS_TARGET_PAIR]-(p:Patent)-[:PUBLISHED_IN]-(y:Year)
OPTIONAL MATCH (p)-[:HAS_ASSIGNEE]-(a:Assignee)
OPTIONAL MATCH (p)-[:HAS_PATENT]-(f:Family)
WITH p, a, f, toInteger(y.year) AS year
WHERE year >= $year_min AND year <= $year_max
RETURN p.pub_no AS pub_no,
       year,
       $value AS target_pair,
       a.name AS assignee,
